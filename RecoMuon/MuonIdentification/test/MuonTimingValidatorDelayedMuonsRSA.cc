// -*- C++ -*-
//
// Package:    MuonTimingValidatorDelayedMuonsRSA
// Class:      MuonTimingValidatorDelayedMuonsRSA
// 
/**\class MuonTimingValidatorDelayedMuonsRSA MuonTimingValidatorDelayedMuonsRSA.cc 
...
// $Id: MuonTimingValidatorDelayedMuonsRSA.cc,v 1.9 2010/03/25 14:08:50 jribnik Exp $
//
//

#include "RecoMuon/MuonIdentification/test/MuonTimingValidatorDelayedMuonsRSA.h"
...
//
MuonTimingValidatorDelayedMuonsRSA::MuonTimingValidatorDelayedMuonsRSA(const edm::ParameterSet& iConfig) 
  :
  refittedStandAloneMuonTag_(iConfig.getUntrackedParameter<edm::InputTag>("refittedStandAloneMuonTag")),
  muonCTag_(iConfig.getUntrackedParameter<edm::InputTag>("Muons")),
...
MuonTimingValidatorDelayedMuonsRSA::~MuonTimingValidatorDelayedMuonsRSA()
{
...
void
MuonTimingValidatorDelayedMuonsRSA::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  edm::Handle<reco::MuonCollection> muonC;
  iEvent.getByLabel(muonCTag_,muonC);
  ...
  std::cout<<"refittedStandAloneMuons.isValid() = "<<refittedStandAloneMuons.isValid()<<", muonC.isValid() = "<<muonC.isValid()<<std::endl;

  if (refittedStandAloneMuons.isValid() && muonC.isValid()) {
    // sort muon tracks by pt
    std::vector<reco::Track> refittedStandAloneMuons_;
	...
	it++) {
      ...
      if (debug) std::cout << " Event: " << iEvent.id() << " Found muon. Pt: " << it->pt() << std::endl;	

      if ((fabs(it->eta())<theMinEta) || (fabs(it->eta())>theMaxEta)) continue;            
      ...
      int imucountMatch = -1;
      for (reco::MuonCollection::const_iterator sa=muonC->begin(); sa!=muonC->end(); ++sa, ++imucount) {
	...
	reco::MuonRef muonR(muonC,imucountMatch);
	...
	if(it->pt()>thePtCut){
	  if (timedt.nDof()>theDtCut) {
	    if(it->hitPattern().dtStationsWithValidHits()>theDtChambersCut){
	      ...
	      if(igoodmucount==1) {	      
		hi_sta_pt->Fill(it->pt());
		hi_sta_phi->Fill(it->phi());
		hi_sta_eta->Fill(it->eta());
	      }
		...
		std::cout << "         DT LS Inverse Beta: " << timedt.invbetaLS() << " +/- " << timedt.invbetaLSError() << std::endl;
	      ...
	      if(debug) std::cout<<"finished ndof histos"<<std::endl;
	      	      
	      hi_dttime_ibt->Fill(timedt.inverseBeta());
	      hi_dttime_ibt_pt->Fill(it->pt(),timedt.inverseBeta());
	      ...
	      sta_phi_dttime_invbetaLS_hist->Fill(it->phi(),timedt.invbetaLS());
	      ...
	      if(debug) std::cout<<"finished first group of histos"<<std::endl;
	      ...
	      if(debug) std::cout<<"finished 2nd group of histos"<<std::endl;
		...
		if (timedt.hitChisq31()>0. && timedt.hitChisq31()<1.0*x/10) nevts_passHitChisq[x]++;
	      ...
	      if(debug) std::cout<<"finished 3rd group of histos"<<std::endl;
		...
		hi_dttime_vtxr_pull->Fill(timedt.timeAtIpOutIn()/timedt.timeAtIpOutInErr());
	      ...
	      if(debug) std::cout<<"about to set tree variables"<<std::endl;
  ...
  }//end of if RSA collection is valid
...
void 
MuonTimingValidatorDelayedMuonsRSA::beginJob()
{
...
void 
MuonTimingValidatorDelayedMuonsRSA::endJob() {
...
float 
MuonTimingValidatorDelayedMuonsRSA::calculateDistance(const math::XYZVector& vect1, const math::XYZVector& vect2) {
...
//
TH1F* MuonTimingValidatorDelayedMuonsRSA::divideErr(TH1F* h1, TH1F* h2, TH1F* hout) {
...
DEFINE_FWK_MODULE(MuonTimingValidatorDelayedMuonsRSA);
